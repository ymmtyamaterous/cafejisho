...
export async function seed(dbInstance?: LibSQLDatabase<typeof schema>) {
  const db = dbInstance ?? createLocalDb();

  // テーブルが未生成の場合はスキップ（マイグレーション前の起動でもエラーにならないよう保護）
  const tableExists = await db.all<{ name: string }>(
    sql`SELECT name FROM sqlite_master WHERE type='table' AND name='course'`,
  );
  if (tableExists.length === 0) {
    console.log("⚠️ テーブルが未生成のためシードをスキップします（マイグレーションを先に実行してください）");
    return;
  }
